...
const ELEMENT_TYPES = {
    ...
    reference: (attrs, child, extraAttrs) => {
...
const toRedactor = (jsonValue) => {
    ...
    if (jsonValue.children) {
        children = Array.from(jsonValue.children).map(exports.toRedactor);
        ...
        if (jsonValue.attrs) {
            ...
            if (allattrs['style'] && jsonValue['type'] !== 'img') {
                Object.keys(allattrs['style']).forEach((key) => {
                    style += `${kebabCase_1.default(key)}: ${allattrs.style[key]};`;
                });
