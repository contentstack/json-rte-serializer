...
let listTypes = ["ol", "ul"];
const elementsToAvoidWithinMarkdownTable = [
  "ol",
  "ul",
  "h1",
  "h2",
...
----------`;
  ...
  reference: (attrsJson: any, child: any): any => {
    if (attrsJson?.["display-type"] === "display") {
      if (attrsJson) {
        let assetName = attrsJson?.["asset-name"]
          ? attrsJson["asset-name"]
          : "enter image description here";
  ...
  if (listTypes.includes(value.type)) {
    ...
    Array.from(value.children).forEach((val: any, index) => {
      if (
        ...
        value.children[index + 1]?.type &&
        listTypes.includes(value.children[index + 1].type)
      ) {
        let liChildren = jsonToMarkdownSerializer(val);
        let nestedListChildren = getOLOrULStringFromJson(
          value.children[index + 1]
        );
        let indentedNestedListChildren = nestedListChildren
          .split("\n")
        ...
        if (value.type === "ol")
          child += `${
            nestedListFound ? index + start - 1 : index + start
          }. ${liChildren}\n`;
...
export const jsonToMarkdownSerializer = (jsonValue: any): string => {
  ...
  if (jsonValue.children) {
    children = Array.from(jsonValue.children).map((child) =>
      jsonToMarkdownSerializer(child)
    );
  ...
  if (ELEMENT_TYPES[jsonValue["type"]]) {
    ...
    if (jsonValue["type"] === "table") {
      ...
      let thead = Array.from(jsonValue["children"]).find(
        (child: any) => child.type && child.type === "thead"
      );
      if (!thead) {
    ...
    if (jsonValue["type"] === "td" || jsonValue["type"] === "th") {
      let NonAllowedTableChild = Array.from(jsonValue["children"]).find(
        (child: any) => elementsToAvoidWithinMarkdownTable.includes(child.type)
      );
